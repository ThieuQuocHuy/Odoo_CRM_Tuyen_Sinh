...
QUnit.module('Crm Kanban Progressbar', {
    beforeEach: function () {
            ...
            models: {
                ...
                'crm_ts.lead': {
                    fields: {
                        ...
                        stage_id: { string: 'Stage', type: 'many2one', relation: 'crm_ts.stage' },
        ...
        target = getFixture();
        setupViewRegistries();
        patchUserWithCleanup({ hasGroup: (group) => group === "crm_ts.group_use_recurring_revenues" });
    },
}, function () {
    QUnit.test("Progressbar: do not show sum of MRR if recurring revenues is not enabled", async function (assert) {
        ...
        await makeView({
            type: "kanban",
            serverData,
            resModel: 'crm_ts.lead',
            groupBy: ['stage_id'],
            arch: `
                <kanban js_class="crm_ts_kanban">
        ...
        const reccurringRevenueNoValues = [...target.querySelectorAll('.o_crm_ts_kanban_mrr_counter_side')].map((elem) => elem.textContent)
