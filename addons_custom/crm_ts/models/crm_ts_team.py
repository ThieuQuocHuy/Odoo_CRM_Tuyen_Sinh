...
class Team(models.Model):
    _name = 'crm_ts.team'
    _inherit = ['mail.alias.mixin', 'crm_ts.team']
    ...
    @api.depends('crm_ts_team_member_ids.assignment_max')
    def _compute_assignment_max(self):
        for team in self:
            team.assignment_max = sum(member.assignment_max for member in team.crm_ts_team_member_ids)

    def _compute_assignment_enabled(self):
        assign_enabled = self.env['ir.config_parameter'].sudo().get_param('crm_ts.lead.auto.assignment', False)
    ...
    def _compute_lead_unassigned_count(self):
        leads_data = self.env['crm_ts.lead']._read_group([
            ('team_id', 'in', self.ids),
            ('type', '=', 'lead'),
            ('user_id', '=', False),
        ], ['team_id'], ['__count'])
    ...
    @api.depends('crm_ts_team_member_ids.lead_month_count', 'assignment_max')
    def _compute_lead_all_assigned_month_count(self):
        for team in self:
            team.lead_all_assigned_month_count = sum(member.lead_month_count for member in team.crm_ts_team_member_ids)
    ...
    def _compute_opportunities_data(self):
        opportunity_data = self.env['crm_ts.lead']._read_group([
            ('team_id', 'in', self.ids),
            ('probability', '<', 100),
            ('type', '=', 'opportunity'),
    ...
    def _constrains_assignment_domain(self):
        for team in self:
            try:
                ...
                if domain:
                    self.env['crm_ts.lead'].search(domain, limit=1)
            except Exception:
    ...
    def unlink(self):
        """ When unlinking, concatenate ``crm_ts.lead.scoring.frequency`` linked to
        the team into "no team" statistics. """
        frequencies = self.env['crm_ts.lead.scoring.frequency'].search([('team_id', 'in', self.ids)])
        if frequencies:
            existing_noteam = self.env['crm_ts.lead.scoring.frequency'].sudo().search([
                ('team_id', '=', False),
            ...
            for frequency in frequencies:
                ...
                if match:
                    ...
                    existing_noteam += self.env['crm_ts.lead.scoring.frequency'].sudo().create({
    ...
    def _alias_get_creation_values(self):
        ...
        values['alias_model_id'] = self.env['ir.model']._get('crm_ts.lead').id
        if self.id:
            if not self.use_leads and not self.use_opportunities:
                values['alias_name'] = False
            values['alias_defaults'] = defaults = literal_eval(self.alias_defaults or "{}")
            has_group_use_lead = self.env.user.has_group('crm_ts.group_use_lead')
    ...
    def _action_assign_leads(self, force_quota=False, creation_delta_days=7):
        ...
        """
        if not (self.env.user.has_group('crm_ts_sales_team.group_sale_manager') or self.env.is_system()):
        ...
        _logger.info(
            ...
            len(self),
            len(self.crm_ts_team_member_ids),
    ...
    def _allocate_leads(self, creation_delta_days=7):
        ...
        :config int crm_ts.assignment.bundle: deprecated
        :config int crm_ts.assignment.commit.bundle: optional config parameter allowing
        ...
        :config float crm_ts.assignment.delay: optional config parameter giving a
        ...
        BUNDLE_HOURS_DELAY = float(self.env['ir.config_parameter'].sudo().get_param('crm_ts.assignment.delay', default=0))
        BUNDLE_COMMIT_SIZE = int(self.env['ir.config_parameter'].sudo().get_param('crm_ts.assignment.commit.bundle', 100))
        ...
        for team in self:
            ...
            leads = self.env["crm_ts.lead"].search(lead_domain)
        ...
        while population:
            ...
            if auto_commit and counter % BUNDLE_COMMIT_SIZE == 0:
                ...
                self.env['crm_ts.lead'].browse(lead_unlink_ids).unlink()
                lead_unlink_ids = set()
                self._cr.commit()
    ...
    def _allocate_leads_deduplicate(self, leads, duplicates_cache=None):
        ...
        # classify leads
        leads_assigned = self.env['crm_ts.lead']  # direct team assign
    ...
    def _assign_and_convert_leads(self, force_quota=False):
        ...
        auto_commit = not getattr(threading.current_thread(), 'testing', False)
        result_data = {}
        commit_bundle_size = int(self.env['ir.config_parameter'].sudo().get_param('crm_ts.assignment.commit.bundle', 100))
        teams_with_members = self.filtered(lambda team: team.crm_ts_team_member_ids)
        quota_per_member = {member: member._get_assignment_quota(force_quota=force_quota) for member in self.crm_ts_team_member_ids}
        counter = 0
        leads_per_team = dict(self.env['crm_ts.lead']._read_group(
        ...
        for team, leads_to_assign_ids in leads_per_team.items():
            members_to_assign = list(team.crm_ts_team_member_ids.filtered(lambda member:
            ...
            if not members_to_assign:
                continue
            result_data.update({
                member: {"assigned": self.env["crm_ts.lead"], "quota": quota_per_member[member]}
            ...
            leads_to_assign = self.env['crm_ts.lead'].browse(leads_to_assign_ids).exists()
            leads_per_member = {
        ...
        for member, member_info in result_data.items():
            _logger.info('-> member %s of team %s: assigned %d/%d leads (%s)', member.id, member.crm_ts_team_id.id, len(member_info["assigned"]), member_info["quota"], member_info["assigned"])
        return result_data
    ...
    def action_opportunity_forecast(self):
        action = self.env['ir.actions.actions']._for_xml_id('crm_ts.crm_ts_lead_action_forecast')
    ...
    def _action_update_to_pipeline(self, action):
        ...
        if not user_team_id:
            ...
            if user_team_id:
                if self.env.user.has_group('crm_ts_sales_team.group_sale_manager'):
                    action['help'] += "<p>%s</p>" % _("""As you are a member of no Sales Team, you are showed the Pipeline of the <b>first team by default.</b>
                                        ...
                                        self.env.ref('crm_ts_sales_team.crm_ts_team_action_config').id)
                else:
                    action['help'] += "<p>%s</p>" % _("""As you are a member of no Sales Team, you are showed the Pipeline of the <b>first team by default.</b>
    ...
    def action_primary_channel_button(self):
        ...
        if self.use_opportunities:
            action = self.env['ir.actions.actions']._for_xml_id('crm_ts.crm_ts_case_form_view_salesteams_opportunity')
            rcontext = {
                'team': self,
            }
            action['help'] = self.env['ir.ui.view']._render_template('crm_ts.crm_ts_action_helper', values=rcontext)
            return action
