...
class TeamMember(models.Model):
    _inherit = 'crm_ts.team.member'
    # assignment
    assignment_enabled = fields.Boolean(related="crm_ts_team_id.assignment_enabled")
    ...
    @api.depends('user_id', 'crm_ts_team_id')
    def _compute_lead_day_count(self):
        ...
        for member in self:
            member.lead_day_count = daily_leads_counts.get((member.user_id.id, member.crm_ts_team_id.id), 0)
    ...
    def _compute_lead_month_count(self):
        ...
        for member in self:
            member.lead_month_count = monthly_leads_counts.get((member.user_id.id, member.crm_ts_team_id.id), 0)

    def _get_lead_from_date(self, date_from, active_test=False):
        return {
            (user.id, team.id): count for user, team, count in self.env['crm_ts.lead'].with_context(active_test=active_test)._read_group(
                [
                    ('date_open', '>=', date_from),
                    ('team_id', 'in', self.crm_ts_team_id.ids),
        ...
        for member in self:
            try:
                ...
                raise exceptions.ValidationError(_(
                    ...
                    user=member.user_id.name, team=member.crm_ts_team_id.name
                ))
