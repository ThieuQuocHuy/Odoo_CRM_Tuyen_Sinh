...
class Lead(models.Model):
    ...
    user_id = fields.Many2one(
        'res.users', string='Nhân viên Tuyển Sinh', default=lambda self: self.env.user,
        domain="[('share', '=', False)]",
    ...
    team_id = fields.Many2one(
        'crm_ts.team', string='Sales Team', check_company=True, index=True, tracking=True,
    ...
    # Student info
    student_name = fields.Char('Tên học sinh', tracking=True)
    
    active = fields.Boolean('Active', default=True, tracking=True)
    type = fields.Selection([
        ('lead', 'Lead'), ('opportunity', 'Opportunity')], required=True, tracking=15, index=True,
        default=lambda self: 'lead' if self.env.user.has_group('crm_ts.group_use_lead') else 'opportunity')
    # Pipeline management
    priority = fields.Selection(
        crm_ts_stage.AVAILABLE_PRIORITIES, string='Priority', index=True,
        default=crm_ts_stage.AVAILABLE_PRIORITIES[0][0])
    stage_id = fields.Many2one(
        'crm_ts.stage', string='Stage', index=True, tracking=True,
        compute='_compute_stage_id', readonly=False, store=True,
        copy=False, group_expand='_read_group_stage_ids', ondelete='restrict',
        domain="['|', ('team_id', '=', False), ('team_id', '=', team_id)]")
    tag_ids = fields.Many2many(
        'crm_ts.tag', 'crm_ts_tag_rel', 'lead_id', 'tag_id', string='Tags',
    ...
    recurring_plan = fields.Many2one('crm_ts.recurring.plan', string="Recurring Plan")
    ...
    lost_reason_id = fields.Many2one(
        'crm_ts.lost.reason', string='Lost Reason',
        index=True, ondelete='restrict', tracking=True)
    # Statistics
    calendar_event_ids = fields.One2many('calendar.event', 'opportunity_id', string='Meetings')
    duplicate_lead_ids = fields.Many2many("crm_ts.lead", compute="_compute_potential_lead_duplicates", string="Potential Duplicate Lead", context={"active_test": False})
    ...
    #Care/Consultation tracking fields
    first_contact_date = fields.Date('Liên lạc lần đầu', tracking=True)
    consultation_date = fields.Date('Tư vấn', tracking=True)
    ts_visit_schedule_date = fields.Date('TS hẹn tham quan', tracking=True)
    ph_visit_schedule_date = fields.Date('PH hẹn tham quan', tracking=True)
    actual_visit_date = fields.Date('Tham quan thực tế', tracking=True)
    ph_fee_appointment_date = fields.Date('PH hẹn đóng phí', tracking=True)
    partial_fee_done = fields.Boolean('Đóng 1 phần phí', tracking=True)
    tuition_completed = fields.Boolean('Hoàn thiện học phí', tracking=True)
    # Parent details (Father / Mother)
    father_name = fields.Char('Họ tên Ba', tracking=True)
    father_phone = fields.Char('Điện thoại', tracking=True)
    father_email = fields.Char('Email', tracking=True)
    father_birth_date = fields.Date('Ngày sinh', tracking=True)
    father_job = fields.Char('Nghề nghiệp', tracking=True)

    mother_name = fields.Char('Họ tên', tracking=True)
    mother_phone = fields.Char('Điện thoại', tracking=True)
    mother_email = fields.Char('Email', tracking=True)
    mother_birth_date = fields.Date('Ngày sinh', tracking=True)
    mother_job = fields.Char('Nghề nghiệp', tracking=True)

    #check input
    test_appointment_date = fields.Date('Ngày hẹn kiểm tra', tracking=True)
    actual_test_date = fields.Date('Ngày kiểm tra thực tế', tracking=True)
    pass_test = fields.Boolean('Đạt kiểm tra', tracking=True)
    confirm_admission = fields.Boolean('BGH xác nhận nhập học', tracking=True)
    admission_date = fields.Date('Ngày nhập học', tracking=True)

    math_point= fields.Float('Điểm toán', tracking=True)
    eng_point= fields.Float('Điểm ngoại ngữ', tracking=True)
    literature_point= fields.Float('Điểm ngữ văn', tracking=True)
    social_cognition= fields.Char('Nhận thức & tương tác xã hội', tracking= True)

    #profile
    enough_profile = fields.Boolean('Đủ hồ sơ', tracking=True)
    admission_application = fields.Boolean('đơn xin nhập học', tracking=True)
    admission_agreement = fields.Boolean('Thỏa thuận nhập học', tracking=True)
    psy_test = fields.Boolean('Bảng điều tra tâm lý', tracking=True)
    birth_certificate = fields.Boolean('Giấy khai sinh', tracking=True)
    household_book = fields.Boolean('Sổ hộ khẩu', tracking=True)
    academic_transcript = fields.Boolean('Học bạ', tracking=True)
    picture_34 = fields.Boolean('Hình 3x4', tracking=True)

    #travel/uniform
    shuttle = fields.Boolean('Quan tâm xe đưa rước', tracking=True)
    direction_travel = fields.Selection([
        ('From Home', 'Từ nhà'),
        ('To Home', 'Về nhà'),
        ('Two Way', 'Hai chiều'),
    ], string= "Số chiều", tracking=True)
    pick_drop_area = fields.Char('Khu vực đón trả', tracking=True)
    quantity_peuniform = fields.Integer('SL đồng phục thể dục', tracking=True)
    size_peuniform = fields.Char('Size đồng phục thể dục', tracking=True)
    quantity_uniform = fields.Integer('SL đồng phục chính khóa', tracking=True)
    size_uniform = fields.Char('Size đồng phục chính khóa', tracking=True)

    _sql_constraints = [
        ('check_probability', 'check(probability >= 0 and probability <= 100)', 'The probability of closing the deal should be between 0% and 100%!')
    ]

    # Custom simple fields for TS quick create/Kanban
    lead_birth_year = fields.Integer('Năm sinh học sinh', tracking=True)
    interested_facility = fields.Char('Cơ sở quan tâm', tracking=True)
    # Extended student profile (new snake_case fields for the custom layout)
    identifier_code = fields.Char(string='Mã định danh', tracking=True)
    # Persisted flag: toggled by the button; reset when stage changes
    student_mode = fields.Boolean(string='Student Mode', default=False)
    show_identifier = fields.Boolean(string='Show Identifier', compute='_compute_show_identifier')
    birth_date = fields.Date(string='Ngày sinh')
    def _selection_birth_year(self):
        current_year = datetime.now().year
        # allow range of 30 years back to one year ahead
        return [(str(y), str(y)) for y in range(current_year - 30, current_year + 2)]
    birth_year = fields.Selection(selection=_selection_birth_year, string='Năm sinh')
    ...
    # Sales stats
    quotation_count = fields.Integer(string='Quotations', compute='_compute_sale_stats')
    order_amount_total = fields.Monetary(string='Orders', currency_field='company_currency', compute='_compute_sale_stats')
    # address extensions
    district_id = fields.Char(string='Quận/Huyện')
    ward_id = fields.Char(string='Phường/Xã')
    # campus/school
    current_school = fields.Char(string='Trường đang theo học')
    ...
    def _compute_team_id(self):
        ...
        for lead in self:
            ...
            team = self.env['crm_ts.team']._get_default_team_id(user_id=user.id, domain=team_domain)
    ...
    def _compute_potential_lead_duplicates(self):
        ...
        for lead in self:
            ...
            duplicate_lead_ids = self.env['crm_ts.lead']

            # check the "company" email domain duplicates
            if lead.email_domain_criterion:
                duplicate_lead_ids |= return_if_relevant('crm_ts.lead', common_lead_domain + [
    ...
    def _compute_is_partner_visible(self):
        """ When the crm_ts.lead is of type 'lead', we don't want to display the "Customer" field on the form view
    ...
    def _auto_init(self):
        super()._auto_init()
        tools.create_index(self._cr, 'crm_ts_lead_user_id_team_id_type_index',
                           self._table, ['user_id', 'team_id', 'type'])
        tools.create_index(self._cr, 'crm_ts_lead_create_date_team_id_idx',
    ...
    def write(self, vals):
        ...
        if 'stage_id' in vals:
            ...
            if stage_updated and vals.get('stage_id'):
                stage = self.env['crm_ts.stage'].browse(vals['stage_id'])
                if stage.is_won:
    ...
    def _handle_won_lost(self, vals):
        ...
        in scoring frequency table."""
        Lead = self.env['crm_ts.lead']
        leads_reach_won = Lead
        leads_leave_won = Lead
        leads_reach_lost = Lead
        leads_leave_lost = Lead
        won_stage_ids = self.env['crm_ts.stage'].search([('is_won', '=', True)]).ids
        for lead in self:
            if 'stage_id' in vals:
    ...
    def copy_data(self, default=None):
        ...
        if not self.env.user.has_group('crm_ts.group_use_recurring_revenues'):
    ...
    def _stage_find(self, team_id=False, domain=None, order='sequence, id', limit=1):
        """ Determine the stage of the current lead with its teams, the given domain and the given team_id
            ...
            :returns crm_ts.stage recordset
        """
        ...
        return self.env['crm_ts.stage'].search(search_domain, order=order, limit=limit)
    ...
    def action_set_lost(self, **additional_values):
        ...
        for lead in self:
            ...
            # ABD : We could have a mixed pipeline, with "lost" stages being separated
            # from "standard" stages. In the future, we may want to prevent any
            # "standard" stage to have a higher sequence than any "lost" stage.
            # But while this is not the case, searching for the "lost" stage while
            # alterning the sequence order (see below) will correctly handle such a case :
            #       stage sequence : [x] [x (lost)] [y] [y (lost)] [z] [z (lost)]
            #       when in stage [y] and marked as "lost", should go to the stage
            #       [y (lost)], not in [x (lost)] nor [z (lost)]
            stage_id = next((stage for stage in lost_stages if stage.sequence > lead.stage_id.sequence), None)
            if not stage_id:
                stage_id = next((stage for stage in reversed(lost_stages) if stage.sequence <= lead.stage_id.sequence),lost_stages)

            if stage_id in leads_by_lost_stage:
                leads_by_lost_stage[stage_id] += lead
            else:
                leads_by_lost_stage[stage_id] = lead
        for lost_stage_id, leads in leads_by_lost_stage.items():
            leads.write({'stage_id': lost_stage_id.id, 'probability': 0})
    ...
    def action_view_student(self):
        self.ensure_one()
        # Validations per business rules before enabling student mode
        # 1) Must be in stage L7, L8 or Won equivalent: rely on name contains keywords (customize if you have codes)
        stage_name = (self.stage_id.name or '').lower()
        in_required_stage = ('đã đóng 1 phần phí' in stage_name) or ('hoàn thiện học phí' in stage_name) or ('won' in stage_name)
        if not in_required_stage:
            raise UserError(_('Chỉ tạo/mở Học sinh khi ở L7, L8 hoặc Won (đóng phí/hoàn thiện học phí/thắng).'))

        # 2) Must have at least one confirmed sale order (not quotation)
        SaleOrder = self.env['sale.order']
        sale_orders = SaleOrder.search([('crm_ts_lead_id', '=', self.id), ('state', 'in', ['sale', 'done'])], limit=1)
        if not sale_orders:
            raise UserError(_('Cần có đơn hàng đã xác nhận (Sale Order), không phải Quotation.'))

        # Tạo mã định danh trực tiếp nếu chưa có
        if not self.identifier_code:
            ...
            # Kiểm tra trùng lặp
            while self.env['crm_ts.lead'].search_count([('id', '!=', self.id), ('identifier_code', '=', random_code)]):
                random_code = str(random.randint(100000, 999999))
        ...
        # Mở popup wizard với random mã định danh
        return {
            'type': 'ir.actions.act_window',
            'name': _('Học sinh'),
            'res_model': 'crm_ts.student.wizard',
            'view_mode': 'form',
            'target': 'new',
            'context': {
                'default_lead_id': self.id,
                'default_lead_name': self.name,
                'default_student_name': self.student_name,
                'default_birth_date': self.birth_date,
                'default_lead_birth_year': self.lead_birth_year,
            }
        }

    @api.depends('stage_id', 'student_mode')
    def _compute_show_identifier(self):
        for lead in self:
            stage_name = (lead.stage_id.name or '').lower()
            # Hiển thị ở L7, L8 và Won
            in_required_stage = ('đã đóng 1 phần phí' in stage_name) or ('hoàn thiện học phí' in stage_name) or ('won' in stage_name)
            lead.show_identifier = bool(in_required_stage and lead.student_mode)

    def write(self, vals):
        # Không reset student_mode khi chuyển stage để giữ mã định danh
        result = super(Lead, self).write(vals)
        return result
    def action_new_quotation(self):
        """Open the Sale Quotation form prefilled from this lead/opportunity."""
        self.ensure_one()
        # Base action from Sales
        action = self.env.ref('sale.action_quotations_with_onboarding').read()[0]
        # Prepare defaults using a fresh context (avoid string context from action)
        ctx = {
            ...
            'default_crm_ts_lead_id': self.id,
            'default_crm_ts_student_name': self.student_name or self.contact_name or False,
            'default_crm_ts_interested_facility': self.interested_facility or False,
        }
        if self.partner_id:
            addresses = self.partner_id.address_get(['invoice', 'delivery'])
            ctx.update({
                'default_partner_invoice_id': addresses.get('invoice'),
                'default_partner_shipping_id': addresses.get('delivery'),
            })
        action['context'] = ctx
        # Force opening the form view as per your screenshot
    ...
    def _compute_sale_stats(self):
        ...
        for lead in self:
            if not lead.id:
                ...
                lead.order_amount_total = 0.0
                continue
                
            domain = [('crm_ts_lead_id', '=', lead.id)]
            
            # Đếm quotations (draft, sent) - nếu có quotation thì hiển thị 1, không có thì 0
            quotation_count = SaleOrder.search_count(domain + [('state', 'in', ['draft', 'sent'])])
            lead.quotation_count = 1 if quotation_count > 0 else 0
            
            # Tính tổng tiền từ TẤT CẢ orders (cả quotation và sales order) - hiển thị số tiền như trong ảnh
            all_orders = SaleOrder.read_group(domain, ['amount_total:sum'], [])
            total_amount = all_orders and all_orders[0].get('amount_total_sum', 0.0) or 0.0
            lead.order_amount_total = total_amount

    def action_refresh_sale_stats(self):
        """Force refresh sale statistics"""
        self._compute_sale_stats()
        return True
    ...
    def action_view_quotations(self):
        self.ensure_one()
        action = self.env.ref('sale.action_quotations_with_onboarding').read()[0]
        action['domain'] = [('crm_ts_lead_id', '=', self.id), ('state', 'in', ['draft', 'sent'])]
        ctx = action.get('context', {})
        if isinstance(ctx, str):
            ctx = eval(ctx) if ctx else {}
        elif not isinstance(ctx, dict):
            ctx = {}
        ctx.update({'default_crm_ts_lead_id': self.id})
        action['context'] = ctx
        action['target'] = 'current'
        return action
    ...
    def action_view_orders(self):
        ...
        # Hiển thị TẤT CẢ orders (cả quotation và sales order)
        action['domain'] = [('crm_ts_lead_id', '=', self.id)]
        ctx = action.get('context', {})
        if isinstance(ctx, str):
            ctx = eval(ctx) if ctx else {}
        elif not isinstance(ctx, dict):
    ...
    def action_show_potential_duplicates(self):
        ...
        """
        self.ensure_one()
        action = self.env["ir.actions.actions"]._for_xml_id("crm_ts.crm_ts_lead_opportunities")
    ...
    def get_empty_list_help(self, help_message):
        ...
        alias_domain = [
            ...
            ('alias_id.alias_name', '!=', ''),
            ('alias_id.alias_model_id.model', '=', 'crm_ts.lead'),
        ]
        # sort by use_leads, then by our membership of the team
        alias_records = self.env['crm_ts.team'].search(alias_domain).sorted(
    ...
    def merge_opportunity(self, user_id=False, team_id=False, auto_unlink=True):
        ...
        :return crm_ts.lead record resulting of th merge
        """
    ...
    def _merge_opportunity(self, user_id=False, team_id=False, auto_unlink=True, max_length=5):
        ...
        if merged_data.get('team_id'):
            team_stage_ids = self.env['crm_ts.stage'].search(['|', ('team_id', '=', merged_data['team_id']), ('team_id', '=', False)], order='sequence, id')
    ...
    def _merge_dependences(self, opportunities):
        ...
        :param opportunities : recordset of opportunities to transfer. Does not
          include `self` which is the target crm_ts.lead being the result of the
          merge;
        """
        self.ensure_one()
    ...
    def _merge_dependences_history(self, opportunities):
        ...
        is the crm_ts.lead record destination for message of `opportunities`.
    ...
    def _merge_dependences_calendar_events(self, opportunities):
        """ Move calender.event from the given opportunities to the current one. `self` is the
            crm_ts.lead record destination for event of `opportunities`.
    ...
    def _merge_followers(self, opportunities):
        ...
        :param opportunities : Record<crm_ts.lead> of opportunities to transfer
        ...
        self.env.cr.execute(
         ...
         LEFT JOIN mail_followers AS destf
                ON destf.res_model = 'crm_ts.lead'
             ...
             WHERE mf.res_model = 'crm_ts.lead'
    ...
    def _merge_log_summary(self, merged_followers, opportunities_tail):
        ...
        self.message_post_with_source(
            "crm_ts.crm_ts_lead_merge_summary",
            render_values={
    ...
    def _handle_salesmen_assignment(self, user_ids=False, team_id=False):
        ...
        if not user_ids and team_id:
            ...
            for idx in range(0, steps):
                ...
                self.env['crm_ts.lead'].browse(subset_ids).write(update_vals)
    ...
    def _creation_subtype(self):
        return self.env.ref('crm_ts.mt_lead_create')
    ...
    def _track_subtype(self, init_values):
        ...
        if 'stage_id' in init_values and self.probability == 100 and self.stage_id:
            return self.env.ref('crm_ts.mt_lead_won')
        elif 'lost_reason_id' in init_values and self.lost_reason_id:
            return self.env.ref('crm_ts.mt_lead_lost')
        elif 'stage_id' in init_values:
            return self.env.ref('crm_ts.mt_lead_stage')
        elif 'active' in init_values and self.active:
            return self.env.ref('crm_ts.mt_lead_restored')
    ...
    def _notify_get_recipients_groups(self, message, model_description, msg_vals=None):
        ...
        salesman_group_id = self.env.ref('crm_ts_sales_team.group_sale_salesman').id
        new_group = (
            'group_sale_salesman',
    ...
    def message_new(self, msg_dict, custom_values=None):
        ...
        if msg_dict.get('priority') in dict(crm_ts_stage.AVAILABLE_PRIORITIES):
    ...
    def get_import_templates(self):
        return [{
            ...
            'template': '/crm_ts/static/xls/crm_ts_lead.xls'
        }]
    ...
    def _pls_get_naive_bayes_probabilities(self, batch_mode=False):
        ...
        frequencies = self.env['crm_ts.lead.scoring.frequency'].search([('variable', 'in', list(leads_fields))], order="team_id asc, id")
    ...
    def _rebuild_pls_frequency_table(self):
        ...
        try:
            ...
            self._cr.execute('TRUNCATE TABLE crm_ts_lead_scoring_frequency')
        ...
        _logger.info("Predictive Lead Scoring : crm_ts.lead.scoring.frequency table rebuilt")
    ...
    def _update_automated_probabilities(self):
        ...
        leads_to_update = self.env['crm_ts.lead'].search(pending_lead_domain)
    ...
    def _pls_prepare_update_frequency_table(self, rebuild=False, target_state=False):
        ...
        if rebuild:  # rebuild is ok
            ...
            team_ids = self.env['crm_ts.team'].with_context(active_test=False).search([]).ids + [0]  # If team_id is unset, consider it as team 0
        else:  # increment
        ...
        if not rebuild:  # there is no existing frequency in rebuild mode as they were all deleted.
            ...
            existing_frequencies = self.env['crm_ts.lead.scoring.frequency'].search_read(
    ...
    def _pls_update_frequency_table(self, new_frequencies_by_team, step, existing_frequencies_by_team=None):
        ...
        LeadScoringFrequency = self.env['crm_ts.lead.scoring.frequency'].sudo()
    ...
    def _pls_get_safe_start_date(self):
        ...
        str_date = self.env['ir.config_parameter'].sudo().get_param('crm_ts.pls_start_date')
    ...
    def _pls_get_safe_fields(self):
        ...
        pls_fields_config = self.env['ir.config_parameter'].sudo().get_param('crm_ts.pls_fields')
    ...
    def _pls_get_won_lost_total_count(self, team_results):
        ...
        first_stage_id = self.env['crm_ts.stage'].search([('team_id', '=', False)], order='sequence, id', limit=1)
    ...
    def _pls_prepare_frequencies(self, lead_values, leads_pls_fields, target_state=None):
        ...
        stage_ids = self.env['crm_ts.stage'].search_read([], ['sequence', 'name', 'id'], order='sequence, id')
    ...
    def _pls_get_lead_pls_values(self, domain=[]):
        ...
        if domain:
            ...
            query = self.env['crm_ts.lead'].with_context(active_test=False)._where_calc(domain)
            table = query.table
            ...
            if use_tags:
                # Get tags values
                tag_rel_alias = query.left_join(table, 'id', 'crm_ts_tag_rel', 'lead_id', 'crm_ts_tag_rel')
                tag_alias = query.left_join(tag_rel_alias, 'tag_id', 'crm_ts_tag', 'id', 'crm_ts_tag')
                self._cr.execute(query.select(
