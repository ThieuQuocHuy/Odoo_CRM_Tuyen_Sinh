...
class IrConfigParameter(models.Model):
    ...
    def write(self, vals):
        ...
        if any(record.key == "crm_ts.pls_fields" for record in self):
            self.env.flush_all()
    ...
    def create(self, vals_list):
        ...
        if any(record.key == "crm_ts.pls_fields" for record in records):
            self.env.flush_all()
            self.env.registry.setup_models(self.env.cr)
        return records

    def unlink(self):
        pls_emptied = any(record.key == "crm_ts.pls_fields" for record in self)
