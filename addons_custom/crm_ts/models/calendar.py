...
class CalendarEvent(models.Model):
    ...
    def default_get(self, fields):
        if self.env.context.get('default_opportunity_id'):
            self = self.with_context(
                default_res_model_id=self.env.ref('crm_ts.model_crm_ts_lead').id,
    ...
    opportunity_id = fields.Many2one(
        'crm_ts.lead', 'Opportunity', domain="[('type', '=', 'opportunity')]",
        index=True, ondelete='set null')

    def _compute_is_highlighted(self):
        super(CalendarEvent, self)._compute_is_highlighted()
        if self.env.context.get('active_model') == 'crm_ts.lead':
    ...
    def _is_crm_ts_lead(self, defaults, ctx=None):
        """
        ...
        return res_model and res_model == 'crm_ts.lead' or res_model_id and self.env['ir.model'].sudo().browse(res_model_id).model == 'crm_ts.lead'
