...
class TestUi(HttpCase, TestCrmCommon):
    ...
    def test_01_crm_ts_tour(self):
        ...
        self.start_tour("/odoo", 'crm_ts_tour', login="admin")

    def test_02_crm_ts_tour_rainbowman(self):
        # we create a new user to make sure they get the 'Congrats on your first deal!'
        # rainbowman message.
        self.env['res.users'].create({
            'name': 'Temporary CRM User',
            'login': 'temp_crm_ts_user',
            'password': 'temp_crm_ts_user',
            'groups_id': [(6, 0, [
                    self.ref('base.group_user'),
                    self.ref('crm_ts_sales_team.group_sale_salesman')
                ])]
        })
        self.start_tour("/odoo", 'crm_ts_rainbowman', login="temp_crm_ts_user")

    def test_03_crm_ts_tour_forecast(self):
        self.start_tour("/odoo", 'crm_ts_forecast', login="admin")
    ...
    def test_email_and_phone_propagation_edit_save(self):
        ...
        """
        self.env['crm_ts.lead'].search([]).unlink()
        ...
        self.start_tour('/odoo', 'crm_ts_email_and_phone_propagation_edit_save', login='admin')
