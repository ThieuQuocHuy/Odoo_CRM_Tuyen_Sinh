...
from odoo.addons.crm_ts.models.crm_ts_lead import PARTNER_FIELDS_TO_SYNC, PARTNER_ADDRESS_FIELDS_TO_SYNC
from odoo.addons.crm_ts.tests.common import TestCrmCommon, INCOMING_EMAIL
...
class TestCRMLead(TestCrmCommon):
    ...
    def test_crm_ts_lead_contact_fields_mixed(self):
        ...
        lead = self.env['crm_ts.lead'].create(lead_data)
        # classic
    ...
    @users('user_sales_leads')
    def test_crm_ts_lead_creation_no_partner(self):
        lead_data = {
            'name': 'Test',
            'country_id': self.country_ref.id,
            'email_from': self.test_email,
        ...
        lead = self.env['crm_ts.lead'].new(lead_data)
    ...
    def test_crm_ts_lead_creation_partner(self):
        lead = self.env['crm_ts.lead'].create({
            'name': 'TestLead',
    ...
    @users('user_sales_manager')
    def test_crm_ts_lead_creation_partner_address(self):
    ...
    @users('user_sales_manager')
    def test_crm_ts_lead_creation_partner_company(self):
    ...
    @users('user_sales_manager')
    def test_crm_ts_lead_creation_partner_no_address(self):
    ...
    def test_crm_ts_lead_create_pipe_data(self):
        ...
        self.assertEqual(lead.team_id, self.env['crm_ts.team'])
    ...
    @users('user_sales_manager')
    def test_crm_ts_lead_currency_sync(self):
        lead_company = self.env['res.company'].sudo().create({
            'name': 'EUR company',
            'currency_id': self.env.ref('base.EUR').id,
        })
        lead = self.env['crm_ts.lead'].with_company(lead_company).create({
            'name': 'Lead 1',
    ...
    @users('user_sales_manager')
    def test_crm_ts_lead_date_closed(self):
        # Test for one won lead
        stage_team1_won2 = self.env['crm_ts.stage'].create({
            'name': 'Won2',
            'sequence': 75,
    ...
    def test_crm_ts_lead_lost_date_closed(self):
    ...
    @users('user_sales_manager')
    def test_crm_ts_lead_meeting_display_fields(self):
        lead = self.env['crm_ts.lead'].create({'name': 'Lead With Meetings'})
    ...
    @users('user_sales_manager')
    def test_crm_ts_lead_partner_sync(self):
    ...
    @users('user_sales_manager')
    def test_crm_ts_lead_partner_sync_email_phone(self):
    ...
    @users('user_sales_manager')
    def test_crm_ts_lead_partner_sync_email_phone_corner_cases(self):
    ...
    @users('user_sales_manager')
    def test_crm_ts_lead_stages(self):
    ...
    @users('user_sales_manager')
    def test_crm_ts_lead_unlink_calendar_event(self):
        ...
        self.assertEqual(meetings.mapped('res_model'), ['crm_ts.lead', 'crm_ts.lead'])
        lead.unlink()
    ...
    @users('user_sales_leads')
    def test_crm_ts_lead_update_contact(self):
    ...
    @users('user_sales_manager')
    def test_crm_ts_team_alias(self):
        new_team = self.env['crm_ts.team'].create({
            'name': 'TestAlias',
            'use_leads': True,
    ...
    @users('user_sales_manager')
    def test_crm_ts_team_alias_helper(self):
        """Test that the help message is the right one if we are on multiple team with different settings."""
        # archive other teams
        self.env['crm_ts.team'].search([]).active = False

        self._activate_multi_company()
        team_other_comp = self.team_company2

        user_team_leads, team_leads, user_team_opport, team_opport = self.env['crm_ts.team'].create([{
            'name': 'UserTeamLeads',
        ...
        self.env['crm_ts.lead'].with_user(self.user_sales_manager_mc).create({
            'name': 'LeadOtherComp',
        ...
        for team in teams:
            with self.subTest(team=team):
                if team != team_other_comp:
                    self.assertIn(f"<a href='mailto:{team.alias_email}'>{team.alias_email}</a>", self.env['crm_ts.lead'].sudo().get_empty_list_help(""))
                else:
                    self.assertNotIn(f"<a href='mailto:{team.alias_email}'>{team.alias_email}</a>", self.env['crm_ts.lead'].sudo().get_empty_list_help(""))
                team.active = False
    ...
    def test_phone_mobile_search(self):
        lead_1 = self.env['crm_ts.lead'].create({
            'name': 'Lead 1',
            'country_id': self.env.ref('base.be').id,
            'phone': '+32485001122',
        })
        lead_2 = self.env['crm_ts.lead'].create({
            'name': 'Lead 2',
            'country_id': self.env.ref('base.be').id,
            'phone': '0032485001122',
        })
        lead_3 = self.env['crm_ts.lead'].create({
            'name': 'Lead 3',
            'country_id': self.env.ref('base.be').id,
            'phone': 'hello',
        })
        lead_4 = self.env['crm_ts.lead'].create({
            'name': 'Lead 3',
            'country_id': self.env.ref('base.be').id,
            'phone': '+32485112233',
        })
        ...
        with self.assertRaises(UserError):
            self.env['crm_ts.lead'].search([('phone_mobile_search', 'like', '')])
        # with self.assertRaises(UserError):
        #     self.env['crm_ts.lead'].search([('phone_mobile_search', 'like', '7   ')])
        with self.assertRaises(UserError):
            self.env['crm_ts.lead'].search([('phone_mobile_search', 'like', 'c')])
        with self.assertRaises(UserError):
            self.env['crm_ts.lead'].search([('phone_mobile_search', 'like', '+')])
        with self.assertRaises(UserError):
            self.env['crm_ts.lead'].search([('phone_mobile_search', 'like', '5')])
        with self.assertRaises(UserError):
            self.env['crm_ts.lead'].search([('phone_mobile_search', 'like', '42')])

        # + / 00 prefixes do not block search
        self.assertEqual(lead_1 + lead_2, self.env['crm_ts.lead'].search([
        ...
        self.assertEqual(lead_1 + lead_2 + lead_4, self.env['crm_ts.lead'].search([
            ('phone_mobile_search', 'like', '1122')
        ]))

        # textual input still possible
        self.assertEqual(
            self.env['crm_ts.lead'].search([('phone_mobile_search', 'like', 'hello')]),
            lead_3,
        ...
        self.assertEqual(
            self.env['crm_ts.lead'].search([('phone_mobile_search', 'ilike', 'Hello')]),
            lead_3,
            'Should behave like a text field (case insensitive)'
        )
        self.assertEqual(
            self.env['crm_ts.lead'].search([('phone_mobile_search', 'like', 'hello123')]),
            self.env['crm_ts.lead'],
            'Should behave like a text field'
        )
    ...
    def test_phone_mobile_search_format(self):
        ...
        self.assertEqual(leads, self.env['crm_ts.lead'].search([
...
class TestLeadFormTools(FormatAddressCase):
    ...
    def test_address_view(self):
        ...
        self.assertAddressView('crm_ts.lead')
...
class TestCrmLeadMailTrackingDuration(MailTrackingDurationMixinCase):
    ...
    @classmethod
    def setUpClass(cls):
        super().setUpClass('crm_ts.lead')

    def test_crm_ts_lead_mail_tracking_duration(self):
        self._test_record_duration_tracking()

    def test_crm_ts_lead_mail_tracking_duration_batch(self):
        self._test_record_duration_tracking_batch()

    def test_crm_ts_lead_queries_batch_mail_tracking_duration(self):
