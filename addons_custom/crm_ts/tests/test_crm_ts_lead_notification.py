...
class NewLeadNotification(TestCrmCommon):
    ...
    def test_lead_message_get_suggested_recipients_values_for_create(self):
        ...
        for partner_name, name, email in [
            ...
            with self.subTest(partner_name=partner_name):
                ...
                description = '<p>Top</p>'
                lead1 = self.env['crm_ts.lead'].create({
                    'name': 'TestLead',
                    'contact_name': name,
    ...
    def test_new_lead_notification(self):
        ...
        sales_team_1 = self.env['crm_ts.team'].create({
            'name': 'Test Sales Team',
            'alias_name': 'test_sales_team',
        })

        subtype = self.env.ref("crm_ts.mt_salesteam_lead")
        ...
        lead = self.env["crm_ts.lead"].with_context(mail_create_nosubscribe=True).sudo().create({
    ...
    def test_new_lead_from_email_multicompany(self):
        ...
        crm_ts_team_model_id = self.env['ir.model']._get_id('crm_ts.team')
        crm_ts_lead_model_id = self.env['ir.model']._get_id('crm_ts.lead')

        crm_ts_team0 = self.env['crm_ts.team'].create({
            'name': 'crm_ts team 0',
            'company_id': company0.id,
        })
        crm_ts_team1 = self.env['crm_ts.team'].create({
            'name': 'crm_ts team 1',
        ...
        mail_alias0 = self.env['mail.alias'].create({
            ...
            'alias_model_id': crm_ts_lead_model_id,
            'alias_parent_model_id': crm_ts_team_model_id,
            'alias_parent_thread_id': crm_ts_team0.id,
            'alias_defaults': "{'type': 'opportunity', 'team_id': %s}" % crm_ts_team0.id,
        })
        mail_alias1 = self.env['mail.alias'].create({
            ...
            'alias_model_id': crm_ts_lead_model_id,
            'alias_parent_model_id': crm_ts_team_model_id,
            'alias_parent_thread_id': crm_ts_team1.id,
            'alias_defaults': "{'type': 'opportunity', 'team_id': %s}" % crm_ts_team1.id,
        })

        crm_ts_team0.write({'alias_id': mail_alias0.id})
        crm_ts_team1.write({'alias_id': mail_alias1.id})
...
--000000000000a47519057e029630--
"""
        crm_ts_lead0_id = self.env['mail.thread'].message_process('crm_ts.lead', new_message0)
        crm_ts_lead1_id = self.env['mail.thread'].message_process('crm_ts.lead', new_message1)

        crm_ts_lead0 = self.env['crm_ts.lead'].browse(crm_ts_lead0_id)
        crm_ts_lead1 = self.env['crm_ts.lead'].browse(crm_ts_lead1_id)

        self.assertEqual(crm_ts_lead0.team_id, crm_ts_team0)
        self.assertEqual(crm_ts_lead1.team_id, crm_ts_team1)

        self.assertEqual(crm_ts_lead0.company_id, company0)
        self.assertEqual(crm_ts_lead1.company_id, company1)
