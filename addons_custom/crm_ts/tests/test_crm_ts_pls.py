...
class TestCRMPLS(TransactionCase):
    ...
    def setUpClass(cls):
        ...
        cls.user_sales_manager = mail_new_test_user(
            ...
            name='Martin PLS Sales Manager', email='crm_ts_manager@test.example.com',
            company_id=cls.company_main.id,
            notification_type='inbox',
            groups='crm_ts_sales_team.group_sale_manager,base.group_partner_manager',
        )

        cls.pls_team = cls.env['crm_ts.team'].create({
            'name': 'PLS Team',
        })

        # Ensure independance on demo data
        cls.env['crm_ts.lead'].with_context({'active_test': False}).search([]).unlink()
        cls.env['crm_ts.lead.scoring.frequency'].search([]).unlink()
        cls.cr.flush()
    ...
    def generate_leads_with_tags(self, tag_ids):
        ...
        team_id = self.env['crm_ts.team'].create({
            'name': 'blup',
        }).id
    ...
    def test_crm_ts_lead_pls_update(self):
        ...
        """
        # Set the PLS config
        frequency_fields = self.env['crm_ts.lead.scoring.frequency.field'].search([])
        pls_fields_str = ','.join(frequency_fields.mapped('field_id.name'))
        pls_start_date_str = "2021-01-01"
        IrConfigSudo = self.env['ir.config_parameter'].sudo()
        IrConfigSudo.set_param("crm_ts.pls_start_date", pls_start_date_str)
        IrConfigSudo.set_param("crm_ts.pls_fields", pls_fields_str)
        ...
        pls_update_wizard = Form(self.env['crm_ts.lead.pls.update'])
        ...
        pls_update_wizard0.action_update_crm_ts_lead_probabilities()

        # Config params should have been updated
        self.assertEqual(IrConfigSudo.get_param("crm_ts.pls_start_date"), date_to_update, 'Correct date is updated in config')
        self.assertEqual(IrConfigSudo.get_param("crm_ts.pls_fields"), fields_after_updation_str, 'Correct fields are updated in config')
    ...
    def test_predictive_lead_scoring(self):
        ...
        Lead = self.env['crm_ts.lead']
        LeadScoringFrequency = self.env['crm_ts.lead.scoring.frequency']
        ...
        stage_ids = self.env['crm_ts.stage'].search([], limit=3).ids
        won_stage_id = self.env['crm_ts.stage'].search([('is_won', '=', True)], limit=1).id
        team_ids = self.env['crm_ts.team'].create([{'name': 'Team Test 1'}, {'name': 'Team Test 2'}, {'name': 'Team Test 3'}]).ids
        ...
        # Set the PLS config
        self.env['ir.config_parameter'].sudo().set_param("crm_ts.pls_start_date", "2000-01-01")
        self.env['ir.config_parameter'].sudo().set_param("crm_ts.pls_fields", "country_id,state_id,email_state,phone_state,source_id,tag_ids")
        ...
        tag_ids = self.env['crm_ts.tag'].create([
            {'name': "Tag_test_1"},
            {'name': "Tag_test_2"},
        ]).ids
        ...
        self.env['ir.config_parameter'].sudo().set_param("crm_ts.pls_fields", False)
        Lead._cron_update_automated_probabilities()
        self.env.invalidate_all()
        ...
        self.env['ir.config_parameter'].sudo().set_param("crm_ts.pls_fields", "country_id,state_id,email_state,phone_state,source_id")
        Lead._cron_update_automated_probabilities()
        self.env.invalidate_all()
    ...
    def test_predictive_lead_scoring_always_won(self):
        ...
        stage_id = self.env['crm_ts.stage'].search([], limit=1).id
        team_id = self.env['crm_ts.team'].create({'name': 'Team Test 1'}).id
        # create two leads
        leads = Lead.create([
        ...
        # set a new tag
        leads.tag_ids = self.env['crm_ts.tag'].create({'name': 'lead scoring edge case'})

        # Set the PLS config
        self.env['ir.config_parameter'].sudo().set_param("crm_ts.pls_start_date", "2000-01-01")
        # tag_ids can be used in versions newer than v14
        self.env['ir.config_parameter'].sudo().set_param("crm_ts.pls_fields", "country_id")
    ...
    def test_settings_pls_start_date(self):
        ...
        set_param("crm_ts.pls_start_date", "2021-10-10")
        ...
        set_param("crm_ts.pls_start_date", "")
        res_config_new = resConfig.new()
        self.assertEqual(Date.to_string(res_config_new.predictive_lead_scoring_start_date),
            str_date_8_days_ago, "If config param is empty, date in settings should be set to 8 days before today")

        set_param("crm_ts.pls_start_date", "One does not simply walk into system parameters to corrupt them")
        res_config_new = resConfig.new()
        self.assertEqual(Date.to_string(res_config_new.predictive_lead_scoring_start_date),
    ...
    def test_pls_no_share_stage(self):
        ...
        Lead = self.env['crm_ts.lead']
        team_id = self.env['crm_ts.team'].create([{'name': 'Team Test'}]).id
        self.env['crm_ts.stage'].search([('team_id', '=', False)]).write({'team_id': team_id})
    ...
    def test_team_unlink(self):
        ...
        pls_team = self.env["crm_ts.team"].browse(self.pls_team.ids)

        # clean existing data
        self.env["crm_ts.lead.scoring.frequency"].sudo().search([('team_id', '=', False)]).unlink()
        ...
        self.env["crm_ts.lead.scoring.frequency"].sudo().create([
        ...
        existing_plsteam = self.env["crm_ts.lead.scoring.frequency"].sudo().create([
            {'variable': variable, 'value': value,
             'won_count': won_count, 'lost_count': lost_count,
             'team_id': pls_team.id,
        ...
        self.assertEqual(
            existing_plsteam.exists(), self.env["crm_ts.lead.scoring.frequency"],
