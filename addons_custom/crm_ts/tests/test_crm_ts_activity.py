...
from odoo.addons.crm_ts.tests.common import TestCrmCommon
...
class TestCrmMailActivity(TestCrmCommon):
    ...
    def test_crm_ts_activity_ordering(self):
        ...
        default_order = self.env['crm_ts.lead']._order
        ...
        search_res = self.env['crm_ts.lead'].search([('id', 'in', test_leads.ids)], limit=5, offset=0, order='id ASC')
        self.assertEqual(search_res.ids, test_leads[:5].ids)
        search_res = self.env['crm_ts.lead'].search([('id', 'in', test_leads.ids)], limit=5, offset=5, order='id ASC')
        ...
        test_leads[0:4].activity_schedule(act_type_xmlid='crm_ts.call_for_demo', user_id=self.user_sales_manager.id, date_deadline=deadline_in1d)
        test_leads[0:3].activity_schedule(act_type_xmlid='crm_ts.initial_contact', date_deadline=deadline_in2d)
        test_leads[5].activity_schedule(act_type_xmlid='crm_ts.initial_contact', date_deadline=deadline_in2d)
        (test_leads[1] | test_leads[3]).activity_schedule(act_type_xmlid='crm_ts.initial_contact', date_deadline=deadline_was1d)
        (test_leads[2] | test_leads[4]).activity_schedule(act_type_xmlid='crm_ts.call_for_demo', date_deadline=deadline_was2d)
        test_leads.invalidate_recordset()

        expected_ids_asc = [2, 4, 1, 3, 5, 0, 8, 7, 9, 6]
        expected_leads_asc = self.env['crm_ts.lead'].browse([test_leads[lid].id for lid in expected_ids_asc])
        expected_ids_desc = [5, 0, 1, 3, 2, 4, 8, 7, 9, 6]
        expected_leads_desc = self.env['crm_ts.lead'].browse([test_leads[lid].id for lid in expected_ids_desc])
        ...
        search_res = self.env['crm_ts.lead'].search(_domain, limit=None, offset=0, order=_order)
        self.assertEqual(expected_leads_asc.ids, search_res.ids)
        search_res = self.env['crm_ts.lead'].search(_domain, limit=4, offset=0, order=_order)
        self.assertEqual(expected_leads_asc[:4].ids, search_res.ids)
        search_res = self.env['crm_ts.lead'].search(_domain, limit=4, offset=3, order=_order)
        self.assertEqual(expected_leads_asc[3:7].ids, search_res.ids)
        search_res = self.env['crm_ts.lead'].search(_domain, limit=None, offset=3, order=_order)
    ...
    def test_crm_ts_activity_recipients(self):
        ...
        activity = self.env['mail.activity'].with_user(self.user_sales_leads).create({
            ...
            'res_model_id': self.env.ref('crm_ts.model_crm_ts_lead').id,
        })
    ...
    def test_crm_ts_activity_next_action(self):
        ...
        lead_model_id = self.env['ir.model']._get('crm_ts.lead').id
