...
from odoo.addons.crm_ts.tests.common import TestLeadConvertMassCommon
...
class TestLeadMerge(TestLeadMergeCommon):
    ...
    def _run_merge_wizard(self, leads):
        res = self.env['crm_ts.merge.opportunity'].with_context({
            'active_model': 'crm_ts.lead',
        ...
        return self.env['crm_ts.lead'].browse(res['res_id'])
    ...
    def test_initial_data(self):
        ...
        self.assertEqual(self.lead_w_partner.stage_id, self.env['crm_ts.stage'])
    ...
    def test_lead_merge_address_not_propagated(self):
        ...
        leads = self.env['crm_ts.lead'].browse(self.leads.ids)._sort_by_confidence_level(reverse=True)
    ...
    def test_lead_merge_internals(self):
        ...
        merge = self.env['crm_ts.merge.opportunity'].with_context({
            'active_model': 'crm_ts.lead',
        ...
        merge_opportunity = self.env['crm_ts.lead'].browse(result['res_id'])
    ...
    def test_lead_merge_probability_auto(self):
        ...
        leads = self.env['crm_ts.lead'].browse((self.lead_1 + self.lead_w_partner + self.lead_w_partner_company).ids)
    ...
    def test_merge_method(self):
        ...
        lost_reason = self.env['crm_ts.lost.reason'].create({'name': 'Test Reason'})
    ...
    def test_merge_method_dependencies(self):
        ...
        lead_w_partner -----------lead---seq=False
        """
        self.env['crm_ts.lead'].browse(self.lead_w_partner_company.ids).write({'type': 'opportunity'})
        ...
        lead_1 = self.env['crm_ts.lead'].browse(self.lead_1.ids)
        activity = lead_1.activity_schedule('crm_ts.lead_test_activity_1')
        calendar_event = self.env['calendar.event'].create({
            'name': 'Meeting with partner',
            'activity_ids': [(4, activity.id)],
            'start': '2021-06-12 21:00:00',
            'stop': '2021-06-13 00:00:00',
            'res_model_id': self.env['ir.model']._get('crm_ts.crm_ts_lead').id,
            'res_id': lead_1.id,
