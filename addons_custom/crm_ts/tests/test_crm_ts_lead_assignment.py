...
from odoo.addons.crm_ts.tests.common import TestLeadConvertCommon
...
class TestLeadAssignCommon(TestLeadConvertCommon):
    ...
    def setUpClass(cls):
        ...
        cls.env['crm_ts.team'].search([('id', 'not in', cls.sales_teams.ids)]).write({'active': False})
        ...
        with mute_logger('odoo.models.unlink'):
            cls.env['crm_ts.lead'].with_context(active_test=False).search(['|', ('team_id', '=', False), ('user_id', 'in', cls.sales_teams.member_ids.ids)]).unlink()
        cls.bundle_size = 50
        cls.env['ir.config_parameter'].set_param('crm_ts.assignment.commit.bundle', '%s' % cls.bundle_size)
        cls.env['ir.config_parameter'].set_param('crm_ts.assignment.delay', '0')
...
class TestLeadAssign(TestLeadAssignCommon):
    ...
    def test_assign_configuration(self):
        ...
        with patch.object(fields.Datetime, 'now', return_value=now_patch):
            config = self.env['res.config.settings'].create({
                'crm_ts_use_auto_assignment': True,
                'crm_ts_auto_assignment_action': 'auto',
                'crm_ts_auto_assignment_interval_number': 19,
                'crm_ts_auto_assignment_interval_type': 'hours'
            })
            config._onchange_crm_ts_auto_assignment_run_datetime()
            config.execute()
            ...
            config.write({
                'crm_ts_auto_assignment_interval_number': 2,
                'crm_ts_auto_assignment_interval_type': 'days'
            })
            config._onchange_crm_ts_auto_assignment_run_datetime()
            config.execute()
            self.assertTrue(self.assign_cron.active)
            ...
            config.write({
                'crm_ts_auto_assignment_run_datetime': fields.Datetime.to_string(datetime(2020, 11, 1, 10, 0, 0)),
            })
            config.execute()
            self.assertTrue(self.assign_cron.active)
            ...
            config.write({
                'crm_ts_auto_assignment_action': 'manual',
            })
            config.execute()
            ...
            config.write({
                'crm_ts_use_auto_assignment': False,
                'crm_ts_auto_assignment_action': 'auto',
            })
            config.execute()
            self.assertFalse(self.assign_cron.active)
    ...
    def test_assign_count(self):
        ...
        leads = self.env['crm_ts.lead'].search([('id', 'in', leads.ids)])  # ensure order
        ...
        with self.with_user('user_sales_manager'):
            self.env['crm_ts.team'].browse(self.sales_team_1.ids)._action_assign_leads(force_quota=True)
    ...
    def test_assign_duplicates(self):
        ...
        with self.with_user('user_sales_manager'):
            self.env['crm_ts.team'].browse(self.sales_teams.ids)._action_assign_leads()
        ...
        new_assigned_leads_wpartner = self.env['crm_ts.lead'].search([
    ...
    def test_assign_populated(self):
        ...
        self.env.ref('crm_ts.ir_cron_crm_ts_lead_assign').write({'interval_type': 'days', 'interval_number': 30})
        # create a third team
        sales_team_3 = self.env['crm_ts.team'].create({
            'name': 'Sales Team 3',
            'sequence': 15,
            'alias_name': False,
            'use_leads': True,
        ...
        sales_team_3_m1 = self.env['crm_ts.team.member'].create({
            'user_id': self.user_sales_manager.id,
            'crm_ts_team_id': sales_team_3.id,
            'assignment_max': 60,
            'assignment_domain': False,
        })
        sales_team_3_m2 = self.env['crm_ts.team.member'].create({
            'user_id': self.user_sales_leads.id,
            'crm_ts_team_id': sales_team_3.id,
            'assignment_max': 60,
            'assignment_domain': False,
        })
        sales_team_3_m3 = self.env['crm_ts.team.member'].create({
        ...
        with self.with_user('user_sales_manager'):
            self.env['crm_ts.team'].browse(sales_teams.ids)._action_assign_leads()

        # teams assign
        leads = self.env['crm_ts.lead'].search([('id', 'in', leads.ids)])
    ...
    def test_assign_specific_won_lost(self):
        ...
        self.sales_team_1.crm_ts_team_member_ids.write({'assignment_max': 45})
        with self.with_user('user_sales_manager'):
            self.env['crm_ts.team'].browse(self.sales_team_1.ids)._action_assign_leads()

        self.assertEqual(leads[0].team_id, self.env['crm_ts.team'], 'Won lead should not be assigned')
        ...
        self.assertEqual(leads[4].team_id, self.env['crm_ts.team'], 'Lost lead should not be assigned')
    ...
    def test_merge_assign_keep_master_team(self):
        ...
        sales_team_dupe = self.env['crm_ts.team'].create({
        ...
        self.env['crm_ts.team.member'].create({
            'user_id': self.user_sales_salesman.id,
            'crm_ts_team_id': sales_team_dupe.id,
            'assignment_max': 10,
            'assignment_domain': "[]",
        })

        master_opp = self.env['crm_ts.lead'].create({
            'name': 'Master',
            'type': 'opportunity',
            'probability': 50,
        ...
        dupe_lead = self.env['crm_ts.lead'].create({
            'name': 'Dupe',
            'type': 'lead',
    ...
    def test_no_assign_if_exceed_max_assign(self):
        ...
        sales_team_4 = self.env['crm_ts.team'].create({
            'name': 'Sales Team 4',
            'sequence': 15,
            'use_leads': True,
        })
        sales_team_4_m1 = self.env['crm_ts.team.member'].create({
            'user_id': self.user_sales_salesman.id,
            'crm_ts_team_id': sales_team_4.id,
            'assignment_max': 30,
        })
