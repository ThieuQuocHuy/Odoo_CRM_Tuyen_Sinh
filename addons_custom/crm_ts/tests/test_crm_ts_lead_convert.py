...
from odoo.addons.crm_ts.tests import common as crm_common
...
class TestLeadConvertForm(crm_common.TestLeadConvertCommon):
    ...
    def test_form_action_default(self):
        ...
        lead = self.env['crm_ts.lead'].browse(self.lead_1.ids)
        customer = self.env['res.partner'].create({
            "name": "Amy Wong",
            "email": '"Amy, PhD Student, Wong" Tiny <AMY.WONG@test.example.com>'
        })

        wizard = Form(self.env['crm_ts.lead2opportunity.partner'].with_context({
...
class TestLeadConvert(crm_common.TestLeadConvertCommon):
    ...
    def test_lead_convert_base(self):
        """ Test base method ``convert_opportunity`` or crm_ts.lead model """
    ...
    def test_lead_convert_base_corner_cases(self):
        """ Test base method ``convert_opportunity`` or crm_ts.lead model with corner
    ...
    def test_lead_convert_corner_cases_crud(self):
        ...
        convert = self.env['crm_ts.lead2opportunity.partner'].with_context({
    ...
    def test_lead_merge_last_created(self):
        ...
        last_lead = self.env['crm_ts.lead'].create({
