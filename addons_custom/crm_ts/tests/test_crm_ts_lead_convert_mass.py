...
@tagged('lead_manage', 'crm_ts_performance', 'post_install', '-at_install')
class TestLeadConvertMass(crm_common.TestLeadConvertMassCommon):
    ...
    def test_assignment_salesmen(self):
        ...
        with self.assertQueryCount(user_sales_manager=0):
            test_leads = self.env['crm_ts.lead'].browse(test_leads.ids)
    ...
    def test_mass_convert_internals(self):
        ...
        mass_convert = self.env['crm_ts.lead2opportunity.partner.mass'].with_context({
    ...
    def test_mass_convert_find_existing(self):
        ...
        lead = self.env['crm_ts.lead'].create({'name': 'Asa Depape'})
        mass_convert = self.env['crm_ts.lead2opportunity.partner.mass'].with_context({
            'active_model': 'crm_ts.lead',
            'active_ids': lead.ids,
