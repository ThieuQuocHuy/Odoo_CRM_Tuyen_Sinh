...
from odoo.addons.crm_ts.models.crm_ts_lead import PARTNER_ADDRESS_FIELDS_TO_SYNC
...
class TestCrmCommon(TestSalesCommon, MailCase):
    ...
    def setUpClass(cls):
        ...
        (cls.user_sales_manager + cls.user_sales_leads + cls.user_sales_salesman).write({
            'groups_id': [(4, cls.env.ref('crm_ts.group_use_lead').id)]
        })

        cls.env['crm_ts.stage'].search([]).write({'sequence': 9999})  # ensure search will find test data first
        cls.stage_team1_1 = cls.env['crm_ts.stage'].create({
            'name': 'New',
            'sequence': 1,
            'team_id': cls.sales_team_1.id,
        })
        cls.stage_team1_2 = cls.env['crm_ts.stage'].create({
            'name': 'Proposition',
            'sequence': 5,
            'team_id': cls.sales_team_1.id,
        })
        cls.stage_team1_won = cls.env['crm_ts.stage'].create({
            'name': 'Won',
            'sequence': 70,
            'team_id': cls.sales_team_1.id,
            'is_won': True,
        })
        cls.stage_gen_1 = cls.env['crm_ts.stage'].create({
            'name': 'Generic stage',
            'sequence': 3,
            'team_id': False,
        })
        cls.stage_gen_won = cls.env['crm_ts.stage'].create({
            'name': 'Generic Won',
            'sequence': 30,
            'team_id': False,
            'is_won': True,
        ...
        # leads
        cls.lead_1 = cls.env['crm_ts.lead'].create({
        ...
        cls.lead_team_1_won = cls.env['crm_ts.lead'].create({
            'name': 'Already Won',
            'type': 'lead',
            'user_id': cls.user_sales_leads.id,
            'team_id': cls.sales_team_1.id,
        ...
        cls.lead_team_1_lost = cls.env['crm_ts.lead'].create({
            'name': 'Already Won',
            'type': 'lead',
            'user_id': cls.user_sales_leads.id,
            'team_id': cls.sales_team_1.id,
    ...
    def _activate_multi_company(cls):
        ...
        cls.team_company2 = cls.env['crm_ts.team'].create({
            'company_id': cls.company_2.id,
            'name': 'C2 Team',
            'sequence': 10,
            'user_id': False,
        })
        cls.team_company2_m1 = cls.env['crm_ts.team.member'].create({
            'crm_ts_team_id': cls.team_company2.id,
            'user_id': cls.user_sales_manager_mc.id,
            'assignment_max': 30,
            'assignment_domain': False,
        })

        cls.team_company1 = cls.env['crm_ts.team'].create({
    ...
    def _create_leads_batch(self, lead_type='lead', count=10, email_dup_count=0,
        ...
        return self.env['crm_ts.lead'].create(leads_data + dups_data)
    ...
    def _create_duplicates(self, lead, create_opp=True):
        ...
        lead_email_from = self.env['crm_ts.lead'].create({
        ...
        lead_email_normalized = self.env['crm_ts.lead'].create({
        ...
        lead_partner = self.env['crm_ts.lead'].create({
        ...
        if create_opp:
            opp_lost = self.env['crm_ts.lead'].create({
    ...
    def assertLeadMerged(self, opportunity, leads, **expected):
        ...
        def _aggregate(fname):
            if isinstance(self.env['crm_ts.lead'][fname], models.BaseModel):
...
class TestLeadConvertCommon(TestCrmCommon):
    ...
    def setUpClass(cls):
        ...
        cls.sales_team_convert = cls.env['crm_ts.team'].create({
        ...
        cls.sales_team_convert_m1 = cls.env['crm_ts.team.member'].create({
            'user_id': cls.user_sales_salesman.id,
            'crm_ts_team_id': cls.sales_team_convert.id,
            'assignment_max': 30,
            'assignment_domain': False,
        })
        cls.stage_team_convert_1 = cls.env['crm_ts.stage'].create({
            'name': 'New',
            'sequence': 1,
        ...
        cls.crm_ts_lead_dt_patcher = patch('odoo.addons.crm_ts.models.crm_ts_lead.fields.Datetime', wraps=Datetime)
        cls.crm_ts_lead_dt_mock = cls.startClassPatcher(cls.crm_ts_lead_dt_patcher)
    ...
    def _switch_to_multi_membership(cls):
        ...
        cls.sales_team_1_m3 = cls.env['crm_ts.team.member'].create({
            'user_id': cls.user_sales_salesman.id,
            'crm_ts_team_id': cls.sales_team_1.id,
            'assignment_max': 15,
        ...
        cls.sales_team_convert_m2 = cls.env['crm_ts.team.member'].create({
            'user_id': cls.user_sales_manager.id,
            'crm_ts_team_id': cls.sales_team_convert.id,
            'assignment_max': 60,
            'assignment_domain': False,
    ...
    def _switch_to_auto_assign(cls):
        cls.env['ir.config_parameter'].set_param('crm_ts.lead.auto.assignment', True)
        cls.assign_cron = cls.env.ref('crm_ts.ir_cron_crm_ts_lead_assign')
    ...
    def assertMemberAssign(self, member, count):
        ...
        member_leads = self.env['crm_ts.lead'].search([
            ('user_id', '=', member.user_id.id),
            ('team_id', '=', member.crm_ts_team_id.id),
...
class TestLeadConvertMassCommon(TestLeadConvertCommon):
    ...
    def setUpClass(cls):
        ...
        cls.user_sales_leads_convert = mail_new_test_user(
            ...
            name='Lucien Sales Leads Convert', email='crm_ts_leads_2@test.example.com',
            company_id=cls.env.ref("base.main_company").id,
            notification_type='inbox',
            groups='crm_ts_sales_team.group_sale_salesman_all_leads,base.group_partner_manager,crm_ts.group_use_lead',
        )
        cls.sales_team_convert_m2 = cls.env['crm_ts.team.member'].create({
        ...
        cls.lead_w_partner = cls.env['crm_ts.lead'].create({
            'name': 'New1',
            'type': 'lead',
            'priority': '0',
            'probability': 10,
        ...
        cls.tags = cls.env['crm_ts.tag'].create([{'name': 'Tag %i' % i} for i in range(4)])
        cls.lead_1.tag_ids = cls.tags[:3]
        cls.lead_w_partner_company = cls.env['crm_ts.lead'].create({
            'name': 'New1',
            'type': 'lead',
            'probability': 50,
            'user_id': cls.user_sales_manager.id,
        ...
        cls.lead_w_contact = cls.env['crm_ts.lead'].create({
            'name': 'LeadContact',
            'type': 'lead',
            'probability': 25,
        ...
        cls.lead_w_email = cls.env['crm_ts.lead'].create({
        ...
        cls.lead_w_email_lost = cls.env['crm_ts.lead'].create({
            'name': 'Lost',
            'type': 'lead',
            'probability': 15,
