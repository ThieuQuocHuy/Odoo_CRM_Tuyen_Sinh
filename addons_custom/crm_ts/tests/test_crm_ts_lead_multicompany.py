...
class TestCRMLeadMultiCompany(TestCrmCommon):
    ...
    def test_lead_mc_company_computation(self):
        ...
        lead_no_team = self.env['crm_ts.lead'].create({
            'name': 'L1',
            'team_id': False,
            'user_id': False,
        })
        ...
        lead_team_c2 = self.env['crm_ts.lead'].create({
            'name': 'L2',
        ...
        lead_team_no_company = self.env['crm_ts.lead'].create({
            'name': 'No company',
    ...
    def test_lead_mc_company_computation_env_team_norestrict(self):
        ...
        LeadUnsyncCids = self.env['crm_ts.lead'].with_context(allowed_company_ids=[self.company_main.id])
    ...
    def test_lead_mc_company_computation_env_user_restrict(self):
        ...
        LeadUnsyncCids = self.env['crm_ts.lead'].with_context(allowed_company_ids=[self.company_main.id, self.company_2.id])
    ...
    def test_lead_mc_company_form(self):
        ...
        crm_ts_lead_form = Form(self.env['crm_ts.lead'])
        crm_ts_lead_form.name = "Test Lead"

        # default values: current user, its team and therefore its company
        self.assertEqual(crm_ts_lead_form.company_id, self.company_2)
        self.assertEqual(crm_ts_lead_form.user_id, self.user_sales_manager_mc)
        self.assertEqual(crm_ts_lead_form.team_id, self.team_company2)

        # remove user, team only
        crm_ts_lead_form.user_id = self.env['res.users']
        self.assertEqual(crm_ts_lead_form.company_id, self.company_2)
        self.assertEqual(crm_ts_lead_form.user_id, self.env['res.users'])
        self.assertEqual(crm_ts_lead_form.team_id, self.team_company2)

        # remove team, user only
        crm_ts_lead_form.user_id = self.user_sales_manager_mc
        crm_ts_lead_form.team_id = self.env['crm_ts.team']
        self.assertEqual(crm_ts_lead_form.company_id, self.company_2)
        self.assertEqual(crm_ts_lead_form.user_id, self.user_sales_manager_mc)
        self.assertEqual(crm_ts_lead_form.team_id, self.env['crm_ts.team'])

        # remove both: void company to ease assignment
        crm_ts_lead_form.user_id = self.env['res.users']
        self.assertEqual(crm_ts_lead_form.company_id, self.env['res.company'])
        self.assertEqual(crm_ts_lead_form.user_id, self.env['res.users'])
        self.assertEqual(crm_ts_lead_form.team_id, self.env['crm_ts.team'])

        # force company manually
        crm_ts_lead_form.company_id = self.company_2
        lead = crm_ts_lead_form.save()
    ...
    def test_lead_mc_company_form_progressives_setup(self):
        ...
        crm_ts_lead_form = Form(lead)
        self.assertEqual(crm_ts_lead_form.company_id, self.env['res.company'])

        crm_ts_lead_form.team_id = self.sales_team_1
        self.assertEqual(crm_ts_lead_form.company_id, self.env['res.company'])

        crm_ts_lead_form.user_id = self.env.user
    ...
    def test_lead_mc_company_form_w_partner_id(self):
        ...
        crm_ts_lead_form.user_id = self.user_sales_manager_mc
        crm_ts_lead_form.partner_id = partner_c2
        self.assertEqual(crm_ts_lead_form.company_id, self.company_2, 'Crm: company comes from sales')
        self.assertEqual(crm_ts_lead_form.team_id, self.team_company2, 'Crm: team comes from sales')
        ...
        self.assertEqual(crm_ts_lead_form.company_id, self.company_2, 'Crm: company comes from partner')
