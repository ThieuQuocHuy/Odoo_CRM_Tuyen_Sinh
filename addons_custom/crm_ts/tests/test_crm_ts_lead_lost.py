...
class TestLeadConvert(crm_common.TestCrmCommon):
    ...
    def setUpClass(cls):
        ...
        cls.lost_reason = cls.env['crm_ts.lost.reason'].create({
            'name': 'Test Reason'
        })
    ...
    def test_lead_lost(self):
        ...
        self.assertEqual(creation_message.subtype_id, self.env.ref('crm_ts.mt_lead_create'))
        ...
        lost_wizard = self.env['crm_ts.lead.lost'].create({
            'lead_ids': lead.ids,
        ...
        update_message = lead.message_ids[0]
        self.assertEqual(update_message.subtype_id, self.env.ref('crm_ts.mt_lead_lost'))
    ...
    def test_lead_lost_batch_wfeedback(self):
        ...
        for lead in leads:
            ...
            lost_message = lead.message_ids.filtered(lambda msg: msg.subtype_id == self.env.ref('crm_ts.mt_lead_lost'))
    ...
    def test_lead_lost_crm_ts_rights(self):
        ...
        with self.assertRaises(AccessError):
            lost_reason = self.env['crm_ts.lost.reason'].create({
                'name': 'Test Reason'
            })
