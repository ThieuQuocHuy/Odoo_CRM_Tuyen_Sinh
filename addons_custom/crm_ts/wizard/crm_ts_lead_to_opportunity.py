...
class Lead2OpportunityPartner(models.TransientModel):
    _name = 'crm_ts.lead2opportunity.partner'
    ...
    def default_get(self, fields):
        ...
        if result.get('lead_id'):
            if self.env['crm_ts.lead'].browse(result['lead_id']).probability == 100:
    ...
    lead_id = fields.Many2one('crm_ts.lead', 'Associated Lead', required=True)
    duplicated_lead_ids = fields.Many2many(
        'crm_ts.lead', string='Opportunities', context={'active_test': False},
    ...
    def _compute_duplicated_lead_ids(self):
        for convert in self:
            if not convert.lead_id:
                ...
                continue
            convert.duplicated_lead_ids = self.env['crm_ts.lead']._get_lead_duplicates(
                convert.partner_id,
    ...
    def _compute_team_id(self):
        ...
        for convert in self:
            ...
            if convert.team_id and user in convert.team_id.member_ids | convert.team_id.user_id:
                continue
            team = self.env['crm_ts.team']._get_default_team_id(user_id=user.id, domain=None)
    ...
    def _action_convert(self):
        """ """
        result_opportunities = self.env['crm_ts.lead'].browse(self._context.get('active_ids', []))
