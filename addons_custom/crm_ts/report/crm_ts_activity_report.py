...
class ActivityReport(models.Model):
    ...
    team_id = fields.Many2one('crm_ts.team', 'Sales Team', readonly=True)
    lead_id = fields.Many2one('crm_ts.lead', "Opportunity", readonly=True)
    ...
    stage_id = fields.Many2one('crm_ts.stage', 'Stage', readonly=True)
    ...
    def _join(self):
        return """
            JOIN crm_ts_lead AS l ON m.res_id = l.id
        """

    def _where(self):
        return """
            WHERE
                m.model = 'crm_ts.lead' AND (m.mail_activity_type_id IS NOT NULL)
        """
