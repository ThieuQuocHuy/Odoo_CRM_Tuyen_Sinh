...
class CrmTeam(models.Model):
    ...
    def _get_default_team_id(self, user_id=False, domain=False):
        ...
        default_team = self.env['crm_ts.team'].browse(
            self.env.context['default_team_id']
        ) if self.env.context.get('default_team_id') else self.env['crm_ts.team']
        ...
        if not team:
            teams = self.env['crm_ts.team'].search([('company_id', 'in', valid_cids)])
    ...
    def action_your_pipeline(self):
        """Return the Pipeline action for CRM TS leads.

        This mirrors the behavior expected by server actions that call
        model.action_your_pipeline().
        """
        return self.env["ir.actions.actions"]._for_xml_id("crm_ts.crm_ts_lead_action_pipeline")
    ...
    crm_ts_team_member_ids = fields.One2many(
        'crm_ts.team.member', 'crm_ts_team_id', string='Sales Team Members',
        context={'active_test': True},
        help="Add members to automatically assign their documents to this sales team.")
    crm_ts_team_member_all_ids = fields.One2many(
        'crm_ts.team.member', 'crm_ts_team_id', string='Sales Team Members (incl. inactive)',
        context={'active_test': False})
    # UX options
    color = fields.Integer(string='Color Index', help="The color of the channel")
    favorite_user_ids = fields.Many2many(
        'res.users', 'crm_ts_team_favorite_user_rel', 'team_id', 'user_id',
    ...
    @api.depends('crm_ts_team_member_ids.active')
    def _compute_member_ids(self):
        for team in self:
            team.member_ids = team.crm_ts_team_member_ids.user_id
    ...
    def _inverse_member_ids(self):
        for team in self:
            ...
            memberships = team.crm_ts_team_member_ids
            users_current = team.member_ids
            users_new = users_current - memberships.user_id

            # add missing memberships
            self.env['crm_ts.team.member'].create([{'crm_ts_team_id': team.id, 'user_id': user.id} for user in users_new])
    ...
    def _compute_member_warning(self):
        ...
        for team in self:
            ...
            other_memberships = self.env['crm_ts.team.member'].search([
                ('crm_ts_team_id', '!=', team._origin.id if team.ids else False),
            ...
            if other_memberships:
                member_warning = _("Adding %(user_names)s in this team will remove them from %(team_names)s.",
                                   ...
                                   team_names=", ".join(other_memberships.mapped('crm_ts_team_id.name'))
                                  )
            if member_warning:
    ...
    def _search_member_ids(self, operator, value):
        return [('crm_ts_team_member_ids.user_id', operator, value)]
    ...
    def write(self, values):
        ...
        if values.get('company_id'):
            self.crm_ts_team_member_ids._check_company(fnames=['crm_ts_team_id'])
    ...
    def _graph_get_model(self) -> str:
        """ skeleton function defined here because it'll be called by crm_ts and/or sale
        """
