...
class CrmTeamMember(models.Model):
    ...
    crm_ts_team_id = fields.Many2one(
        'crm_ts.team', string='Sales Team',
    ...
    @api.constrains('crm_ts_team_id', 'user_id', 'active')
    def _constrains_membership(self):
        ...
        existing = self.env['crm_ts.team.member'].search([
            ('crm_ts_team_id', 'in', self.crm_ts_team_id.ids),
            ('user_id', 'in', self.user_id.ids),
            ('active', '=', True)
        ])
        duplicates = self.env['crm_ts.team.member']

        active_records = dict(
            (membership.user_id.id, membership.crm_ts_team_id.id)
            for membership in self if membership.active
        )
        for membership in self:
            potential = existing.filtered(lambda m: m.user_id == membership.user_id and \
                m.crm_ts_team_id == membership.crm_ts_team_id and m.id != membership.id
            )
            ...
            if active_records.get(potential.user_id.id):
                ...
                active_records[potential.user_id.id] = potential.crm_ts_team_id.id

        if duplicates:
            raise exceptions.ValidationError(
                _("You are trying to create duplicate membership(s). We found that %(duplicates)s already exist(s).",
                  duplicates=", ".join("%s (%s)" % (m.user_id.name, m.crm_ts_team_id.name) for m in duplicates)
                 ))

    @api.depends('crm_ts_team_id', 'is_membership_multi', 'user_id')
    @api.depends_context('default_crm_ts_team_id')
    def _compute_user_in_teams_ids(self):
        ...
        if all(m.is_membership_multi for m in self):
            ...
            member_user_ids = self.env['crm_ts.team.member'].search([('id', 'not in', self.ids)]).user_id
        else:
            member_user_ids = self.env['crm_ts.team.member'].search([]).user_id
        for member in self:
            if member_user_ids:
                member.user_in_teams_ids = member_user_ids
            elif member.crm_ts_team_id:
                member.user_in_teams_ids = member.crm_ts_team_id.member_ids
            elif self.env.context.get('default_crm_ts_team_id'):
                member.user_in_teams_ids = self.env['crm_ts.team'].browse(self.env.context['default_crm_ts_team_id']).member_ids
            else:
                member.user_in_teams_ids = self.env['res.users']

    @api.depends('crm_ts_team_id')
    def _compute_user_company_ids(self):
        all_companies = self.env['res.company'].search([])
        for member in self:
            member.user_company_ids = member.crm_ts_team_id.company_id or all_companies

    @api.depends('crm_ts_team_id')
    def _compute_is_membership_multi(self):
        multi_enabled = self.env['ir.config_parameter'].sudo().get_param('crm_ts_sales_team.membership_multi', False)
        self.is_membership_multi = multi_enabled

    @api.depends('is_membership_multi', 'active', 'user_id', 'crm_ts_team_id')
    def _compute_member_warning(self):
        ...
        if all(m.is_membership_multi for m in self):
            ...
            if not active:
                return
            existing = self.env['crm_ts.team.member'].search([('user_id', 'in', active.user_id.ids)])
            user_mapping = dict.fromkeys(existing.user_id, self.env['crm_ts.team'])
            for membership in existing:
                user_mapping[membership.user_id] |= membership.crm_ts_team_id

            for member in active:
                teams = user_mapping.get(member.user_id, self.env['crm_ts.team'])
                remaining = teams - (member.crm_ts_team_id | member._origin.crm_ts_team_id)
                if remaining:
    ...
    def create(self, values_list):
        ...
        (tracked fields).
        """
        is_membership_multi = self.env['ir.config_parameter'].sudo().get_param('crm_ts_sales_team.membership_multi', False)
    ...
    def write(self, values):
        ...
        if not is_membership_multi and values.get('active'):
            self._synchronize_memberships([
                dict(user_id=membership.user_id.id, crm_ts_team_id=membership.crm_ts_team_id.id)
                for membership in self
            ])
    ...
    def _synchronize_memberships(self, user_team_ids):
        ...
        :param user_team_ids: list of pairs (user_id, crm_ts_team_id)
        """
        existing = self.search([
            ('active', '=', True),  # explicit search on active only, whatever context
            ('user_id', 'in', [values['user_id'] for values in user_team_ids])
        ])
        user_memberships = dict.fromkeys(existing.user_id.ids, self.env['crm_ts.team.member'])
        for membership in existing:
            user_memberships[membership.user_id.id] += membership

        existing_to_archive = self.env['crm_ts.team.member']
        for values in user_team_ids:
            existing_to_archive += user_memberships.get(values['user_id'], self.env['crm_ts.team.member']).filtered(
                lambda m: m.crm_ts_team_id.id != values['crm_ts_team_id']
            )
