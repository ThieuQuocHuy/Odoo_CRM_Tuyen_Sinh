...
class SalesTeamCommon(BaseCommon):
    ...
    def setUpClass(cls):
        ...
        cls.group_sale_salesman = cls.env.ref('crm_ts_sales_team.group_sale_salesman')
        cls.group_sale_manager = cls.env.ref('crm_ts_sales_team.group_sale_manager')
        ...
        cls.sale_team = cls.env['crm_ts.team'].create({
            'name': 'Test Sales Team',
        })
        # Disable other teams (demo data/existing data)
        cls.env['crm_ts.team'].search([
            ('id', '!=', cls.sale_team.id),
        ]).action_archive()
...
class TestSalesCommon(TransactionCase):
    ...
    def setUpClass(cls):
        ...
        cls.env['ir.config_parameter'].set_param('crm_ts_sales_team.membership_multi', False)
        ...
        cls.user_sales_manager = mail_new_test_user(
            ...
            name='Martin Sales Manager', email='crm_ts_manager@test.example.com',
        ...
        cls.user_sales_leads = mail_new_test_user(
            ...
            name='Laetitia Sales Leads', email='crm_ts_leads@test.example.com',
            company_id=cls.company_main.id,
            notification_type='inbox',
            groups='crm_ts_sales_team.group_sale_salesman_all_leads,base.group_partner_manager',
        )
        cls.user_sales_salesman = mail_new_test_user(
            cls.env, login='user_sales_salesman',
            name='Orteil Sales Own', email='crm_ts_salesman@test.example.com',
            company_id=cls.company_main.id,
            notification_type='inbox',
            groups='crm_ts_sales_team.group_sale_salesman',
        )

        cls.env['crm_ts.team'].search([]).write({'sequence': 9999})
        cls.sales_team_1 = cls.env['crm_ts.team'].create({
            'name': 'Test Sales Team',
            'sequence': 5,
            'company_id': False,
            'user_id': cls.user_sales_manager.id,
        })
        cls.sales_team_1_m1 = cls.env['crm_ts.team.member'].create({
            'user_id': cls.user_sales_leads.id,
            'crm_ts_team_id': cls.sales_team_1.id,
        })
        cls.sales_team_1_m2 = cls.env['crm_ts.team.member'].create({
...
class TestSalesMC(TestSalesCommon):
    ...
    def setUpClass(cls):
        ...
        cls.team_c2 = cls.env['crm_ts.team'].create({
            'name': 'C2 Team1',
            'sequence': 1,
            'user_id': False,
            'company_id': cls.company_2.id,
        })
        cls.team_mc = cls.env['crm_ts.team'].create({
