...
from odoo.addons.crm_ts_sales_team.tests.common import TestSalesMC
...
class TestCornerCases(TransactionCase):
    ...
    def setUp(self):
        ...
        self.sales_team_1 = self.env['crm_ts.team'].create({
    ...
    def test_unicity(self):
        ...
        sales_team_1_m1 = self.env['crm_ts.team.member'].create({
            'user_id': self.user_sales_leads.id,
            'crm_ts_team_id': self.sales_team_1.id,
        })

        sales_team_1_m1.write({'active': False})
        sales_team_1_m1.flush_recordset()

        sales_team_1_m2 = self.env['crm_ts.team.member'].create({
            'user_id': self.user_sales_leads.id,
            'crm_ts_team_id': self.sales_team_1.id,
        })

        found = self.env['crm_ts.team.member'].search([
            ('user_id', '=', self.user_sales_leads.id),
            ('crm_ts_team_id', '=', self.sales_team_1.id),
        ])
    ...
    def test_unicity_multicreate(self):
        ...
        with self.assertRaises(exceptions.UserError), mute_logger('odoo.sql_db'):
            self.env['crm_ts.team.member'].create([
                {'user_id': self.user_sales_leads.id, 'crm_ts_team_id': self.sales_team_1.id},
                {'user_id': self.user_sales_leads.id, 'crm_ts_team_id': self.sales_team_1.id}
            ])
...
class TestSecurity(TestSalesMC):
    ...
    def test_team_access(self):
        ...
        crm_ts_sales_team.read(['name'])
        for member in crm_ts_sales_team.member_ids:
            member.read(['name'])

        with self.assertRaises(exceptions.AccessError):
            crm_ts_sales_team.write({'name': 'Trolling'})

        for membership in crm_ts_sales_team.crm_ts_team_member_ids:
