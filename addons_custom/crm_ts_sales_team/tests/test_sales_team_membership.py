...
from odoo.addons.crm_ts_sales_team.tests.common import TestSalesCommon
...
class TestMembership(TestSalesCommon):
    ...
    def setUpClass(cls):
        ...
        cls.new_team = cls.env['crm_ts.team'].create({
            'name': 'Test Specific',
            'sequence': 10,
        })
        cls.env['ir.config_parameter'].set_param('crm_ts_sales_team.membership_multi', True)
    ...
    def test_fields(self):
        ...
        self.env['ir.config_parameter'].sudo().set_param('crm_ts_sales_team.membership_multi', False)
    ...
    def test_members_mono(self):
        ...
        self.env.flush_all()
        memberships = self.env['crm_ts.team.member'].with_context(active_test=False).search([('user_id', '=', self.user_sales_leads.id)])
        self.assertEqual(len(memberships), 3)  # subscribed twice to new_team + subscribed to sales_team_1
        self.assertEqual(memberships.crm_ts_team_id, sales_team_1 | new_team)
        self.assertFalse(memberships.filtered(lambda m: m.crm_ts_team_id == sales_team_1).active)
        new_team_memberships = memberships.filtered(lambda m: m.crm_ts_team_id == new_team)
        ...
        with self.assertRaises(exceptions.UserError):
            self.env['crm_ts.team.member'].create({'crm_ts_team_id': new_team.id, 'user_id': new_user.id})
    ...
    def test_memberships_mono(self):
        """ Test mono mode: updating crm_ts_team_member_ids field """
        self.env['ir.config_parameter'].sudo().set_param('crm_ts_sales_team.membership_multi', False)
        # ensure initial data
        sales_team_1 = self.env['crm_ts.team'].browse(self.sales_team_1.ids)
        new_team = self.env['crm_ts.team'].browse(self.new_team.ids)
        self.assertEqual(sales_team_1.member_ids, self.user_sales_leads | self.user_admin)

        # subscribe on new team (user_sales_leads will have two memberships -> old one deactivated)
        self.assertEqual(new_team.member_ids, self.env['res.users'])
        new_team.write({'crm_ts_team_member_ids': [
        ...
        self.assertEqual(memberships.crm_ts_team_id, sales_team_1 | new_team)
        self.assertFalse(memberships.filtered(lambda m: m.crm_ts_team_id == sales_team_1).active)
        self.assertTrue(memberships.filtered(lambda m: m.crm_ts_team_id == new_team).active)

        # subscribe user_sales_leads on old team -> old membership still archived and kept
        sales_team_1.write({'crm_ts_team_member_ids': [(0, 0, {'user_id': self.user_sales_leads.id})]})
        memberships_new = self.env['crm_ts.team.member'].with_context(active_test=False).search([('user_id', '=', self.user_sales_leads.id)])
        self.assertTrue(memberships < memberships_new)
        self.assertEqual(memberships.crm_ts_team_id, sales_team_1 | new_team)

        # old membership is still inactive, new membership is active
        old_st_1 = memberships_new.filtered(lambda m: m.crm_ts_team_id == sales_team_1 and m in memberships)
        new_st_1 = memberships_new.filtered(lambda m: m.crm_ts_team_id == sales_team_1 and m not in memberships)
        new_nt = memberships_new.filtered(lambda m: m.crm_ts_team_id == new_team)
    ...
    def test_memberships_multi(self):
        ...
        memberships = self.env['crm_ts.team.member'].with_context(active_test=False).search([('user_id', '=', self.user_sales_leads.id)])
        self.assertEqual(memberships.crm_ts_team_id, sales_team_1 | new_team)
        self.assertTrue(memberships.filtered(lambda m: m.crm_ts_team_id == sales_team_1).active)
        self.assertTrue(memberships.filtered(lambda m: m.crm_ts_team_id == new_team).active)

        # archive membership on sales_team_1 and try creating a new one
        memberships.filtered(lambda m: m.crm_ts_team_id == sales_team_1).write({'active': False})
        # subscribe user_sales_leads on old team -> old membership still archived and kept
        sales_team_1.write({'crm_ts_team_member_ids': [(0, 0, {'user_id': self.user_sales_leads.id})]})
        memberships_new = self.env['crm_ts.team.member'].with_context(active_test=False).search([('user_id', '=', self.user_sales_leads.id)])
        self.assertTrue(memberships < memberships_new)
    ...
    def test_memberships_sync(self):
        ...
        new_team = self.env['crm_ts.team'].browse(self.new_team.ids)
        self.assertEqual(sales_team_1.member_ids, self.user_sales_leads | self.user_admin)
        self.assertEqual(new_team.crm_ts_team_member_ids, self.env['crm_ts.team.member'])
        self.assertEqual(new_team.crm_ts_team_member_all_ids, self.env['crm_ts.team.member'])
        self.assertEqual(new_team.member_ids, self.env['res.users'])

        # creating memberships correctly updates m2m without any refresh
        new_member = self.env['crm_ts.team.member'].create({
            'user_id': self.env.user.id,
            'crm_ts_team_id': self.new_team.id,
        })
        self.assertEqual(new_team.crm_ts_team_member_ids, new_member)
        self.assertEqual(new_team.crm_ts_team_member_all_ids, new_member)
        self.assertEqual(new_team.member_ids, self.env.user)
        ...
        added = self.env['crm_ts.team.member'].search([('crm_ts_team_id', '=', new_team.id), ('user_id', '=', self.user_sales_leads.id)])
        self.assertEqual(new_team.crm_ts_team_member_ids, new_member + added)
        self.assertEqual(new_team.crm_ts_team_member_all_ids, new_member + added)
        self.assertEqual(new_team.member_ids, self.env.user | self.user_sales_leads)
        ...
        admin_original = self.env['crm_ts.team.member'].search([
            ('crm_ts_team_id', '=', sales_team_1.id),
            ('user_id', '=', self.user_admin.id)
        ])
        self.assertTrue(bool(admin_original))
        admin_archived = self.env['crm_ts.team.member'].create({
            'crm_ts_team_id': new_team.id,
            'user_id': self.user_admin.id,
            'active': False,
        })
        admin_original.write({'crm_ts_team_id': new_team.id})
        ...
        with self.assertRaises(exceptions.UserError), mute_logger('odoo.sql_db'):
            added.write({'crm_ts_team_id': sales_team_1.id})

    def test_users_sale_team_id(self):
        self.assertTrue(self.sales_team_1.sequence < self.new_team.sequence)

        self.assertEqual(self.user_sales_leads.crm_ts_team_ids, self.sales_team_1)
        ...
        self.assertEqual(self.user_sales_leads.crm_ts_team_ids, self.sales_team_1 | self.new_team)
        self.assertEqual(self.user_sales_leads.sale_team_id, self.sales_team_1)

        # archive membership to first team -> second one becomes default
        self.sales_team_1_m1.write({'active': False})
        self.assertEqual(self.user_sales_leads.crm_ts_team_ids, self.new_team)
