...
from odoo.addons.crm_ts_sales_team.tests.common import SalesTeamCommon, TestSalesCommon, TestSalesMC
...
class TestDefaultTeam(TestSalesCommon):
    ...
    def setUpClass(cls):
        ...
        cls.team_sequence = cls.env['crm_ts.team'].create({
            'company_id': False,
        ...
        cls.team_responsible = cls.env['crm_ts.team'].create({
    ...
    def test_default_team_fallback(self):
        ...
        self.assertFalse(self.env['crm_ts.team.member'].search([('user_id', '=', self.user_sales_leads.id)]))

        # default is better sequence matching company criterion
        with self.with_user('user_sales_leads'):
            team = self.env['crm_ts.team']._get_default_team_id()
    ...
    def test_default_team_wcontext(self):
        ...
        with self.with_user('user_sales_leads'):
            ...
            team = self.env['crm_ts.team'].with_context(
...
class TestMultiCompany(TestSalesMC):
    ...
    def test_team_members(self):
        ...
        team_c2 = self.env['crm_ts.team'].browse(self.team_c2.id)
    ...
    def test_team_memberships(self):
        ...
        # can add someone from same company
        self.env.user.write({'company_id': self.company_2.id})
        team_c2.write({'crm_ts_team_member_ids': [(0, 0, {'user_id': self.env.user.id})]})
        self.assertEqual(team_c2.member_ids, self.env.user)

        # cannot add someone from another company
        with self.assertRaises(exceptions.UserError):
            team_c2.write({'crm_ts_team_member_ids': [(0, 0, {'user_id': self.user_sales_salesman.id})]})
...
class TestAccessRights(SalesTeamCommon):
    ...
    def test_access_sales_manager(self):
        ...
        india_channel = self.env['crm_ts.team'].with_context(tracking_disable=True).create({
            'name': 'India',
        })
        self.assertIn(
            india_channel.id, self.env['crm_ts.team'].search([]).ids,
